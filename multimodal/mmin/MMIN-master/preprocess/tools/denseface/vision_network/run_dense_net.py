...
if __name__ == '__main__':
  ...
  parser.add_argument(
    ...
    help='Train the ete_model')
  parser.add_argument(
    '--test', action='store_true',
    help='Test ete_model for required dataset if pretrained ete_model exists.'
       'If provided together with `--train` flag testing will be'
       'performed right after training.')
  parser.add_argument(
    '--model_type', '-m', type=str, choices=['DenseNet', 'DenseNet-BC'],
    default='DenseNet',
    help='What type of ete_model to use')
  parser.add_argument(
  ...
  parser.add_argument(
    ...
    help='reduction Theta at transition layer for DenseNets-BC ete_model')
  ...
  parser.add_argument(
    ...
    help='Save ete_model during training')
  parser.add_argument(
    '--no-saves', dest='should_save_model', action='store_false',
    help='Do not save ete_model during training')
  parser.set_defaults(should_save_model=True)

  parser.add_argument(
    '--renew-logs', dest='renew_logs', action='store_true',
    help='Erase previous logs for ete_model if exists.')
  parser.add_argument(
    '--not-renew-logs', dest='renew_logs', action='store_false',
    help='Do not erase previous logs for ete_model if exists.')
  ...
  print("Initialize the ete_model..")
  ...
  if args.restore_path is not None:
    ...
    # ete_model.saver.restore(ete_model.sess, args.restore_path)
    print('Successfully restore ete_model from', args.restore_path)
  if args.train:
