...
class DenseNet:
  def __init__(self, data_provider, growth_rate, depth,
    ...
    Args:
      ...
      model_type: `str`, 'DenseNet' or 'DenseNet-BC'. Should ete_model use
        bottle neck connections or not.
      dataset: `str`, dataset name
      should_save_logs: `bool`, should logs be saved or not
      should_save_model: `bool`, should ete_model be saved or not
      renew_logs: `bool`, remove previous logs for current ete_model
    ...
    if not bc_mode:
      print("Build %s ete_model with %d blocks, "
  ...
  def _initialize_session(self):
    ...
    # restrict ete_model GPU memory utilization to min required
  ...
  def save_path(self):
    try:
      ...
      save_path = os.path.join(self.output_dir, self.model_identifier, 'ete_model')
  ...
  def load_model(self):
    try:
      ...
      raise IOError("Failed to to load ete_model "
              "from save path: %s" % self.save_path)
    self.saver.restore(self.sess, self.save_path)
    print("Successfully load ete_model from save path: %s" % self.save_path)
