...
class DensefaceExtractor(object):
    def __init__(self, restore_path=None, mean=131.0754, std=47.858177, device=0, smooth=False):
        """ extract densenet feature
            Parameters:
            ------------------------
            ete_model: ete_model class returned by function 'load_model'
        """
        if restore_path is None:
            restore_path = '/data2/zjm/emo_tools/FER_models/denseface/DenseNet-BC_growth-rate12_depth100_FERPlus/ete_model/epoch-200'
    ...
    def load_model(self, restore_path):
        ...
        print("Successfully load ete_model from ete_model path: {}".format(model_path))
        return model
