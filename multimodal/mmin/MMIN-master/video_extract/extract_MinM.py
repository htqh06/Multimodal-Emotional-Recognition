...
def extract_video_segment(video_path, start_time, end_time, output_folder, identifier):
    ...
    try:
        ...
        # 裁剪视频并保留音频流
        video = input_video.video.filter('crop', w=crop_width, h=height, x=0, y=0)
...
def process_videos(timestamp_folder, video_folder, output_folder):
    """处理每个视频和对应的时间戳文件，只裁剪男性说话部分（视频的右侧）。"""
    for timestamp_file in os.listdir(timestamp_folder):
        if timestamp_file.endswith(".lab") and 'M' in timestamp_file:  # 只处理包含 'M' 的时间戳文件
            ...
            with open(os.path.join(timestamp_folder, timestamp_file), 'r') as f:
                for line in f:
                    ...
                    if len(parts) == 3 and parts[2].startswith('Ses') and 'M' in parts[2]:  # 确保行格式正确且是男性标识符
...
if __name__ == "__main__":
    timestamp_folder = 'D:/Graduation_IEMOCAP/IEMOCAP_full_release/Session2/dialog/lab/Ses02_M/'
